function [X] = alm_R2MC(Obs,idx,Q,r,lambda,mu1,mu2,maxIter)




%mu1 = 0.5;mu2 = 0.01;
tol = 1e-3;tolProj = 1e-1;
...
for i=1:maxIter
    B1 = Obs + E - 1/mu1*Y1; B2 = Z+1/mu2*Y2; tau = 1/(mu1+mu2); A = (mu1*B1+mu2*B2)/(mu1+mu2);
    [U Sig V] = lansvd(A, r, 'L');
...
while ~converged       
    ...
    while primal_converged == false && iter2 <1000
        ...
        temp_X = U(:,1:svp)*diag(diagSig(1:svp)-1/mu2)*V(:,1:svp)';    
        
        E = Omega_c.*(temp_X-M+(1/mu1)*Y1);
        
        Z = (mu2*temp_X - Y2)/(lambda*Q*Q'+mu2*I);
       
        for i=1:100
            deltaZ = lambda*Z*Q*Q'/norm(Z*Q,'fro')+mu2*(Z - temp_X+ Y2/mu2);
            Z_new = Z - 0.1*deltaZ;
            if norm(Z - Z_new, 'fro') < tolProj*norm(Z,'fro')
                break;
            end
            Z = Z_new;
        end
